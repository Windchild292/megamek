...
public class Board implements Serializable, IBoard {
    ...
    public Board(int width, int height, IHex[] data) {
        ...
        for (int y = 0; y < height; y++) {
            if (width >= 0) {
                System.arraycopy(data, (y * width), this.data, (y * width), width);
            }
        }
    }
    ...
    public Enumeration<Coords> getHexesAtDistance(Coords coords, int distance) {
        ...
        Vector<Coords> retVal = new Vector<>();
                    ...
                    if (!bldgByCoords.containsKey(coords)) {
                        ...
                        try {
                            ...
                            if (errBuff == null) {
                                ...
                                errBuff.append("Unable to create building at ")
                                        .append(coords.toString()).append("!\n")
                    ...
                    if (!bldgByCoords.containsKey(coords)) {
                        ...
                        try {
                            ...
                            if (errBuff == null) {
                                ...
                                errBuff.append("Unable to create fuel tank at ")
                                        .append(coords.toString()).append("!\n")
                                        .append(excep.getMessage()).append("\n");
                            }
                    ...
                    if (!bldgByCoords.containsKey(coords)) {
                        ...
                        try {
                            ...
                            if (errBuff == null) {
                                ...
                                errBuff.append("Unable to create bridge at ")
                                        .append(coords.toString()).append("!\n")
                                        .append(excep.getMessage()).append("\n");
                            }
    ...
    public void setHexes(List<Coords> coords, List<IHex> hexes) {
        ...
        if (coords.size() != hexes.size()) {
            throw new IllegalStateException("setHexes received two collections differing size!");
        }
    ...
    public static boolean boardIsSize(final File filepath, final BoardDimensions size) {
        int boardX = 0;
        int boardY = 0;
        try (Reader r = new FileReader(filepath); Reader br = new BufferedReader(r)) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                if (!valid && (errBuff == null)) {
                    ...
                    errBuff.append("Hex ").append(String.format("%1$02d%02$2d", x + 1, y + 1))
                            .append(" is invalid!\n")
                            .append(currBuff.toString().replaceAll("^", "\t"));
                }
            }
        }
        return true;
        ...
        try (Writer w = new OutputStreamWriter(os)) {
            ...
            for (int i = 0; i < data.length; i++) {
                ...
                StringBuilder hexBuff = new StringBuilder("hex ")
                        .append(new Coords(i % width, i / width).getBoardNum()).append(" ")
                        .append(hex.getLevel()).append(" \"");
     ...
     * @param hits   the <code>int</code> number of rounds that hit. If a negative number is passed,
     ...
     * burning inferno.
     *
     * @param coords the <code>Coords</code> being checked. This value must not
     ...
     * @throws ClassNotFoundException if there is a class not found exception
     */
