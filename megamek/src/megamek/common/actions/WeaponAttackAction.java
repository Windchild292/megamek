...
public class WeaponAttackAction extends AbstractAttackAction implements Serializable {
    ...
    public static ToHitData toHit(Game game, int attackerId, Targetable target, int weaponId,
        ...
        return toHit(game, attackerId, target, weaponId, aimingAt, aimingMode, isNemesisConfused,
     ...
     */
    private static ToHitData toHit(Game game, int attackerId, Targetable target, int weaponId,
                                   int aimingAt, AimingMode aimingMode, boolean isNemesisConfused,
                                   boolean exchangeSwarmTarget, Targetable oldTarget,
    ...
    private static String toHitIsImpossible(Game game, Entity ae, int attackerId, Targetable target, int ttype,
        ...
        if (weapon != null && wtype != null) {
            ...
            if (wind.isTornadoF13() && wtype.hasFlag(WeaponType.F_MISSILE)
                    && !game.getBoard().inSpace()) {
                return Messages.getString("WeaponAttackAction.NoMissileTornado");
            } else if (wind.isTornadoF4() && !game.getBoard().inSpace()
    ...
    private static ToHitData compileEnvironmentalToHitMods(Game game, Entity ae, Targetable target, WeaponType wtype,
        ...
        int weatherMod = game.getPlanetaryConditions().getWeather().getHitPenalty(ae);
        if ((weatherMod != 0) && !game.getBoard().inSpace()) {
            weatherToHitMods.addModifier(weatherMod, game.getPlanetaryConditions().getWeather().toString());
        }
            ...
            switch (wind) {
                case MODERATE_GALE:
                    if ((wtype != null) && wtype.hasFlag(WeaponType.F_MISSILE)) {
                        weatherToHitMods.addModifier(1, wind.toString());
                    }
                    break;
                case STRONG_GALE:
                    if ((wtype != null) && wtype.hasFlag(WeaponType.F_BALLISTIC)) {
                        ...
                        weatherToHitMods.addModifier(2, wind.toString());
                    }
                    break;
                case STORM:
                    if ((wtype != null) && wtype.hasFlag(WeaponType.F_BALLISTIC)) {
                        ...
                        weatherToHitMods.addModifier(3, wind.toString());
                    }
                    break;
                case TORNADO_F13:
        ...
        if (game.getPlanetaryConditions().isEMI() && !ae.isConventionalInfantry()) {
