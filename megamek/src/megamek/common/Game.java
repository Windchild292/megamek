...
public class Game implements Serializable {
     ...
     */
    public @Nullable Team getTeamForPlayer(IPlayer p) {
        for (Team team : teams) {
 ...
 // TODO: Correctly implement "Captured" Entities
    ...
    /**
     * Returns an <code>Enumeration</code> of the enemy's active entities at the given coordinates.
     *
    ...
    /**
     * Returns an <code>Enumeration</code> of friendly active entities at the given coordinates.
     *
     * @param c the <code>Coords</code> of the hex being examined.
     ...
     */
    public @Nullable GameTurn removeFirstTurnFor(Entity entity) {
        if (!phase.isMovement()) {
            MegaMek.getLogger().error("Unexpected game phase of " + phase + " in Game::removeFirstTurnFor");
            return null;
        }
        // ignored
        for (int i = turnIndex; i < turnVector.size(); i++) {
    ...
    public void removeTurnFor(Entity entity) {
        ...
        if (getOptions().booleanOption(OptionsConstants.INIT_INF_MOVE_MULTI)
                && (entity instanceof Infantry) && phase.isMovement()) {
        ...
        if (getOptions().booleanOption(OptionsConstants.ADVGRNDMOV_VEHICLE_LANCE_MOVEMENT)
                && (entity instanceof Tank) && phase.isMovement()) {
        ...
        if (getOptions().booleanOption(OptionsConstants.ADVGRNDMOV_MEK_LANCE_MOVEMENT)
                && (entity instanceof Mech) && phase.isMovement()) {
            if ((getMechsLeft(entity.getOwnerId()) % getOptions()
                ...
                // contrived, but may come up e.g. one mech accidentally kills
                // another
                if (hasMoreTurns()) {
                    GameTurn nextTurn = turnVector.elementAt(turnIndex + 1);
                    if (nextTurn instanceof GameTurn.EntityClassTurn) {
    ...
    /**
     * Returns an Enumeration of telemissile attacks scheduled for the end of the physical phase.
     */
    ...
    /**
     * Returns the telemissile vector. Do not modify. >:[ Used for sending all charges to the client.
     */
    ...
    /**
     * Get Entities that have an iNarc Nemesis pod attached and are situated between two Coords
     *
     * @param attacker The attacking <code>Entity</code>.
     * @param target The <code>Coords</code> of the original target.
     * @return an <code>Enumeration</code> of entities that have nemesis pods attached and are
    ...
    private void checkPositionCacheConsistency() {
        ...
        if ((entitiesInCacheCount != entityVectorSize) && !getPhase().isDeployment()
                && !getPhase().isExchange() && !getPhase().isLounge() && !getPhase().isInitiative()
