...
/**
 * Subclass of the roll tracker for <code>AbstractRandom</code> entropy sources.
 * 
 * @author Suvarov454
...
public class MMRoll extends Roll {
     ...
     * 
     * @param rng the {@link AbstractRandom} that produces random numbers.
     */
    public MMRoll(AbstractRandom rng) {
        super(6, 1);
     ...
     * 
     * @param rng the {@link AbstractRandom} that produces random numbers.
     * @param max the smallest <code>int</code> value that is higher than all rolls; all rolls will
     *           be in the value set [0, max).
     */
    public MMRoll(AbstractRandom rng, int max) {
        super(max, 0);
        this.total = rng.randomInt(this.faces) + this.min;
        all.addElement(this.total);
    }
     ...
     */
    public MMRoll(AbstractRandom rng, int count, int start) {
        super(count, start);
        this.total = rng.randomInt(this.faces) + this.min;
        all.addElement(this.total);
    }
   ...
   /**
    * Create a set of virtual dice with the given number of faces that start with the given value,
    * where only a subset of the highest will be kept.
    * 
    * @param rng the {@link AbstractRandom} that produces random numbers.
    * @param count the <code>int</code> number of results possible on each virtual die.
    * @param start the <code>int</code> value that is the start of the value set of each virtual die.
    ...
    */
    public MMRoll(AbstractRandom rng, int count, int start, int keep) {
        super(count, start);
        this.total = rng.randomInt(this.faces) + this.min;
        all.addElement(this.total);
        this.keep = keep;
     ...
     * @param rng the {@link AbstractRandom} that produces random numbers.
     */
    public void addRoll(AbstractRandom rng) {
     ...
     *            third is the starting number (defaults to one for six sided dice, zero for anything else).
     */
    public static void main(String... args) {
        ...
        int sides = 6;
        int start = 1;
        RandomizationMethod randomizationMethod = RandomizationMethod.RANDOM;
        try {
        ...
        // Generate the RNG
        AbstractRandom rng = randomizationMethod.getGenerator();
